...
void main() {
    if (u_hasTexture) {
        ...
        // Diffuse
        vec3 diffuse = light.diffuse * (max(dot(normalizee(vertexNormal), normalize(-light.direction)), 0.0)) * texture(u_textureSampler, vec3(fragmentUV, u_textureArrayLayer)).rgb;

        // Specular
        float shininess = 32;
        vec3 specular = light.specular * pow(max(dot(normalize(iCameraPosition - fragmentPosition), reflect(-lightDirection, fragmentNormal)), 0.0), shininess) * texture(u_textureSampler, vec3(fragmentUV, u_textureArrayLayer)).rgb;
